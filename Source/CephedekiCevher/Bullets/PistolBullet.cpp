...
APistolBullet::APistolBullet(){
    ammoCapsule = CreateDefaultSubobject<USphereComponent>(TEXT("PistolBulletCapsule"));
    ...
    ammoMesh = CreateDefaultSubobject<UStaticMeshComponent>(TEXT("PistolBullet"));
    ...
    BulletTrail = CreateDefaultSubobject<UParticleSystemComponent>(TEXT("PistolBulletTrail"));
    BulletTrail->SetupAttachment(ammoMesh);
    BulletTrail->SetRelativeRotation(FRotator(0.0f, -90.0f, 0.0f));

    BulletAudioCurve = CreateDefaultSubobject<UAudioComponent>(TEXT("PistolBulletAudiuCurve"));
    BulletAudioCurve->SetupAttachment(ammoMesh);

    projectileMovement = CreateDefaultSubobject<UProjectileMovementComponent>(TEXT("PistolProjectileMovement"));
    projectileMovement->SetUpdatedComponent(GetRootComponent());
    projectileMovement->InitialSpeed = 6500;
    projectileMovement->MaxSpeed = 6500;
    projectileMovement->bShouldBounce = true;
    projectileMovement->ProjectileGravityScale = 0.2;

    static ConstructorHelpers::FObjectFinder<UStaticMesh> BulletMesh(TEXT("/Game/_Weapons/Meshes/Ammunition/SM_Shell_9mm"));
    static ConstructorHelpers::FObjectFinder<UParticleSystem> GetBulletTrail(TEXT("/Game/_FX/P_Pistol_Tracer_01"));
...
void APistolBullet::BeginPlay(){
    Super::BeginPlay();
    ammoCapsule->OnComponentHit.AddDynamic(this, &APistolBullet::BulletHit);
    BulletWidget = CreateWidget<UBulletHitWidget>(this->GetWorld()->GetFirstPlayerController(), TBulletWidget);
}

void APistolBullet::EndPlay(EEndPlayReason::Type EndPlayReason){
    Super::EndPlay(EndPlayReason);
    BulletWidget->RemoveFromParent();
}

void APistolBullet::BulletHit(UPrimitiveComponent *HitComponent, AActor *OtherActor, UPrimitiveComponent *OtherComp, FVector NormalImpulse, const FHitResult &Hit){
    ...
    if(ASoldier *soldier = Cast<ASoldier>(OtherActor)){
        ...
        if(NPC->GetNPCsIsEnemy()){
            print("DUSMAN");
            NPC->SetNPCHealth(12);
            if(NPC->GetNPCHealth() <= 28 || NPC->GetNPCHealth() <= 0){
                NPC->DeathEnemy();
...
void APistolBullet::BulletHitAndHide(){
    ...
    GetWorld()->GetTimerManager().SetTimer(BulletTimerHandle, this, &APistolBullet::DestroyBullet, 0.3, false);
}
void APistolBullet::BulletImpactParticle(UPrimitiveComponent *OtherComp, const FHitResult &Hit){
